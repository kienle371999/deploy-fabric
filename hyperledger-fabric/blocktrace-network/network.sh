...
function createConsortium() {
  ...
  set -x
  configtxgen -profile BlocktraceOrdererGenesis -channelID sys-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function yaml_configtx_generate {
    sed -e "s/\${ORDER_PORT}/$1/" \
        -e "s/\${ORG1_PORT}/$2/" \
        -e "s/\${ORG2_PORT}/$3/" \
...
function networkUp() {
  ...
  infoln "Generate configtx.yaml"
  echo "$(yaml_configtx_generate ${ORDER_PORT} ${ORG1_PORT} ${ORG2_PORT})" > configtx/configtx.yaml
...
function packageChainCode() {
  scripts/packageChainCode.sh $CC_NAME $CC_SRC_PATH $CC_VERSION

  if [ $? -ne 0 ]; then
    fatalln "Packaging chaincode failed"
  fi
...
COMPOSE_FILE_BASE=docker/docker-compose-blocktrace-net.yaml
