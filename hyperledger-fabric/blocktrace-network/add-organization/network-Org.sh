...
function packageCC() {
  docker exec ${ORG_NAME_UPPER}cli ./scripts/org-scripts/packageCC_Org.sh $CC_NAME $CC_SRC_PATH $CC_VERSION $ORG_NAME_UPPER $ORG_PORT $ORG1_PORT $ORG2_PORT $ORDER_PORT $CONTRACT_PATH
}

function deployCC() {
  docker exec ${ORG_NAME_UPPER}cli ./scripts/org-scripts/deployCC_Org.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $ORG_NAME_UPPER $ORG_PORT $ORG1_PORT $ORG2_PORT $ORDER_PORT $CONTRACT_PATH
}

# import default enviroments
...
elif [ "$MODE" == "packageCC" ]; then
  infoln "Packaging chaincode on channel '${CHANNEL_NAME}'"
